  ...
  try {
    ...
    if (copy.snapshot && Array.isArray(copy.snapshot.bunkerTimers)) {
      ...
      for (let m = 0; m < maxLoop; m++) {
        ...
        // We'll compute per-layer remainingSeconds by preferring stored remaining (adjusted by layer.lastUpdated),
        // else initialSeconds (adjusted by savedAt), else computed value from percent->seconds.
        for (let li = 0; li < layerList.length; li++) {
          const layer = layerList[li] || {};
          const btLayer = (btimer && Array.isArray(btimer.layers) && btimer.layers[li]) ? btimer.layers[li] : null;

          // stored values in DB (if present)
          const storedRemaining = (btLayer && btLayer.remainingSeconds != null) ? Number(btLayer.remainingSeconds) : null;
          const storedInitial = (btLayer && btLayer.initialSeconds != null) ? Number(btLayer.initialSeconds) : null;
          const layerLastUpdated = (btLayer && btLayer.lastUpdated) ? new Date(btLayer.lastUpdated) : null;
          ...
          let initialSecondsToWrite = null;

          if (storedRemaining != null) {
            // If DB has stored remainingSeconds, adjust it by elapsed since layerLastUpdated if present, else by savedAt.
            const elapsedForLayer = layerLastUpdated ? Math.max(0, (now.getTime() - layerLastUpdated.getTime()) / 1000) : elapsedSinceSaved;
            remainingSeconds = Math.max(0, Number(storedRemaining) - elapsedForLayer);
            // pick initial to write: prefer storedInitial else compute as remaining + elapsedForLayer
            initialSecondsToWrite = (storedInitial != null) ? storedInitial : (remainingSeconds + elapsedForLayer);
          } else if (storedInitial != null) {
            // we have an initialSeconds recorded in DB (assumed to be at savedAt)
            remainingSeconds = Math.max(0, Number(storedInitial) - elapsedSinceSaved);
            initialSecondsToWrite = Number(storedInitial);
          } else if (computedInit > 0) {
            // no stored timer; compute from percent -> seconds and subtract elapsedSinceSaved
            remainingSeconds = Math.max(0, computedInit - elapsedSinceSaved);
            initialSecondsToWrite = computedInit;
          } else {
            // nothing to compute — empty layer
            remainingSeconds = 0;
            initialSecondsToWrite = null;
          }

          // round/clamp
          if (isFinite(remainingSeconds)) remainingSeconds = Number(Number(remainingSeconds).toFixed(6));
          else remainingSeconds = 0;

          // compute remainingPercent relative to initialSecondsToWrite (avoid divide by zero)
          let remainingPercent = 0;
          if (initialSecondsToWrite != null && initialSecondsToWrite > 0) {
            remainingPercent = (remainingSeconds / initialSecondsToWrite) * (initialPercent || 100);
          } else {
            remainingPercent = Number(layer.percent != null ? layer.percent : (layer.initialPercent || 0));
          }
          remainingPercent = Math.max(0, Number(Number(remainingPercent || 0).toFixed(6)));

          // Update client-visible layer fields (so snapshot.clientBunkers[*].layers[*] is adjusted)
          layer.remainingSeconds = (remainingSeconds == null ? null : Number(remainingSeconds));
          layer.percent = remainingPercent;

          // Write back into copy.snapshot.bunkerTimers for canonical returned structure.
          copy.snapshot.bunkerTimers[m].layers[li] = copy.snapshot.bunkerTimers[m].layers[li] || {};
          if (initialSecondsToWrite != null) copy.snapshot.bunkerTimers[m].layers[li].initialSeconds = Number(initialSecondsToWrite);
          // write canonical remainingSeconds (client expects this)
          copy.snapshot.bunkerTimers[m].layers[li].remainingSeconds = Number(remainingSeconds);

          // optional: preserve lastUpdated if present in DB (do not change here)
          if (btLayer && btLayer.lastUpdated) copy.snapshot.bunkerTimers[m].layers[li].lastUpdated = btLayer.lastUpdated;
        } // per-layer loop

        // recompute bunker remaining seconds as sum of layer.remainingSeconds (if available)
        ...
        if (sumRemaining > 0) {
          ...
          // fallback: try to compute from btimer.initialSeconds minus elapsedSinceSaved (older code path)
